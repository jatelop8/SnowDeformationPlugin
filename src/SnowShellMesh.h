...
namespace SnowDeform
{
	...
	private:
		...
		// v610：129² → 181²（用户 2026-08-30 反馈"雪堆三角感明显棱角大"）——129² 间距 16
		// 一个脚印（半径 18-24）只覆盖 2-3 顶点 → 几何台阶/三角面明显。181² 间距 11.4
		// （2048/180）是 uint16 极限内最大密度（180×180×2=64800 tris < 65535，v196 255²
		// 溢出教训）；顶点 32761 < 65536 ✓。边界无缝由 v544 场方案（3 圈淡出到 origZ）保证，
		// 不依赖 129² 与 65² 的顶点对齐（v213 时代方案）。帧率：dirty 帧顶点循环 ~2× 成本
		// （v197 门控只在盖章帧全量，静息帧跳过），用户实测不满意可回退 129。
		std::uint32_t      highResDim = 181;          // 网格密度（顶点数 = dim²）
